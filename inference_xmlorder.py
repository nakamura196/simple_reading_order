...
class OcrInferencer:
    ...
    def _infer(self, single_outputdir_data):
        ...
        for img_path in single_outputdir_data['img_list']:
            ...
            single_image_file_output_sorted=[]
            ...
            if self.cfg['proc_range']['end'] > 2:
                ...
                for single_data_output in single_image_file_output:
                    ...
                    main_txt, cap_txt, sorted_xml = self._sort_result_txt_and_xml(single_data_output['xml'])
                    sum_main_txt += main_txt + '\n'
                    sum_cap_txt += sum_cap_txt + '\n'
                    single_data_output['xml']=sorted_xml
                    single_image_file_output_sorted.append(single_data_output)
            ...
            pred_list_sorted.extend(single_image_file_output_sorted)
    ...
    def _create_result_txt(self, xml_data):
        ...
        xml_data :
            1ページ分の推論結果を持つxmlデータ。
        """
        main_txt,cap_txt,xml_data=self._sort_result_txt_and_xml(xml_data)
        """main_txt = ''
        cap_txt = ''
    ...
    def _sort_result_txt_and_xml(self, xml_data):
        #print(ET.dump(xml_data))
        page_xml=xml_data.find('PAGE')
        width=int(page_xml.attrib['WIDTH'])
        height=int(page_xml.attrib['HEIGHT'])
        #print(width,height)
        tmpcoordlist=[]
        for raworder,line_xml in enumerate(page_xml.iter('LINE')):
            typeat=line_xml.attrib['TYPE']
            ...
            if lh>lw:
                verticalflag=1
            tmpcoordlist.append([xmin,ymin,xmax,ymax,verticalflag,text,typeat,raworder])
        coordlist=[]
        for t1 in tmpcoordlist:
            xmin1,ymin1,xmax1,ymax1,verticalflag1,text1,typeat1,raworder1=t1
            lxmargin,rxmargin,tymargin,bymargin=[2,2,2,2]
            lxcnt,rxcnt,tycnt,bycnt=[0,0,0,0]
            for t2 in tmpcoordlist:
                xmin2,ymin2,xmax2,ymax2,verticalflag2,text2,typeat2,raworder2=t2
                if t1==t2:
                    continue
                if xmin1>xmax2:
                    lxcnt+=1
            ...
            coordlist.append([xmin1/width,ymin1/height,(xmin1+xmax1)/(width*2),
                            ...
                            lxcnt,rxcnt,tycnt,bycnt,verticalflag1,text1,typeat1,raworder1])
        coorddf = pd.DataFrame(coordlist, columns = ['xminp', 'yminp','xcp', 'ycp',
        'widthp','heightp',"lxmargin","rxmargin","tymargin","bymargin","lxcnt","rxcnt","tycnt","bycnt","verticalflag","text","type","raworder"])
        ...
        coorddf["order"]=res
        coorddf.sort_values(['order'],ascending=False,inplace=True)
        coorddf.reset_index(drop=True,inplace=True)
        coorddf["orderindex"]=coorddf.index
        for raworder,line_xml in enumerate(page_xml.iter('LINE')):
            nowcoord=coorddf[coorddf["raworder"]==raworder].iloc[0,]
            #print(line_xml.attrib['STRING'],nowcoord["text"],nowcoord)
            line_xml.attrib.pop("STRING", None)
            line_xml.text=nowcoord["text"]
            line_xml.set('ROD', str(nowcoord["orderindex"])) 
        #print(ET.dump(xml_data))
        #print(coorddf[["text","order"]])
        return "\n".join(coorddf["text"].tolist()),"\n".join(coorddf[coorddf["type"]=="キャプション"]["text"].tolist()),xml_data
